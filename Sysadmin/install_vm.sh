#!/bin/bash
echo "PS1='\[\033[38;5;11m\]\u\[$(tput sgr0)\]\[\033[38;5;15m\]@\h:\[$(tput sgr0)\]\[\033[38;5;6m\][\w]:\[$(tput sgr0)\]\[\033[38;5;15m\] \[$(tput sgr0)\]'" >> /root/.bashrc
MAC=`ip a | grep link/ether | sed -e 's/([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})//g' | cut -d " " -f6 | sed -e 's/[^0-9]//g'`
MAC=`echo "${MAC: -4}"`
echo "vps-$MAC" > /etc/hostname

apt update && apt upgrade -y && apt install -y linuxlogo curl pwgen sudo vim rsync wget fail2ban && linuxlogo > /etc/motd
