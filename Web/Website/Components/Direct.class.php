...
class Direct {
    ...
    /**
     * Twig-like style : replaces the Twig tags with the data provided in $data.
     * 
     * @param type $content The HTML content (or any text content).
     * @param array $data Associative array to replace the content tags. Ex: ["mytwig" => "My twig !"]
     * @return 
     */
    public function renderView($content, $data) {
        $content_processed = $content;
        foreach ($data as $key_name => $key_val) {
            $content_processed = preg_replace("#{{( )?" . $key_name . "( )?}}#", $key_val, $content_processed);
        }
